...
public class CloudSettingsAPI {
    ...
    private static HttpGet get(final String url) {
        if (checkLogin()) {
            ...
            get.addHeader(HttpHeaders.USER_AGENT, String.format("cloud settings mod (1.18 %s)", CloudSettings.MOD_VERSION));
            return get;
        }
    ...
    private static HttpPost post(final String url) {
        if (checkLogin()) {
            ...
            post.addHeader(HttpHeaders.USER_AGENT, String.format("cloud settings mod (1.18 %s)", CloudSettings.MOD_VERSION));
            return post;
        }
        return null;
    }
